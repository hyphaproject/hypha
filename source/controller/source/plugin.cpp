...
void hypha::controller::Plugin::add(std::string id, std::string host,
...
void hypha::controller::Plugin::remove(std::string id) {
  database->getSession() << "DELETE FROM `plugins` WHERE id='" + id + "';",
      Poco::Data::Keywords::now;
}

void hypha::controller::Plugin::updateConfig(std::string id,
                                             std::string config) {
