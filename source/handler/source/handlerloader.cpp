...
HandlerLoader *HandlerLoader::instance() {
  ...
  if (!singleton) {
    ...
    if (!singleton)
      singleton =
          new HandlerLoader(hypha::settings::HandlerSettings::instance());
    mutex.unlock();
  }
...
void HandlerLoader::loadLocalInstances() {
  for (std::string id : settings->getLocalHandlerIds()) {
    if (getHandlerInstance(id) == 0) {
      ...
      if (handler) handlerInstances[id] = handler;
    }
  }
}
...
std::list<HyphaHandler *> HandlerLoader::getHandlers() { return handlers; }

std::list<HyphaHandler *> HandlerLoader::getInstances() {
  std::list<HyphaHandler *> retList;
  for (std::map<std::string, HyphaHandler *>::const_iterator it =
           handlerInstances.begin();
       it != handlerInstances.end(); ++it) {
...
std::list<std::string> HandlerLoader::getConnectedPlugins(
