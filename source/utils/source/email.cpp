...
void EMail::setHost(std::string host) {
    this->host = host;
}
...
void EMail::setUser(std::string user) {
    this->user = user;
}

void EMail::setPassword(std::string password) {
    this->password = password;
}

bool EMail::sendMessage(std::string to, std::string subject, std::string content) {
...
bool EMail::sendMessageWithAttachment(std::string to, std::string subject, std::string content, std::string filename, std::string file) {
    subject = Poco::Net::MailMessage::encodeWord(subject, "UTF-8");
    Poco::Net::MailMessage message;
    message.setSender(this->user);
    message.addRecipient(Poco::Net::MailRecipient(Poco::Net::MailRecipient::PRIMARY_RECIPIENT, to));
