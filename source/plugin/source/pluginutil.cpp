...
bool PluginUtil::isHandler(HyphaBasePlugin *plugin) {
  if (dynamic_cast<HyphaHandler *>(plugin)) return true;
  return false;
}

bool PluginUtil::isActor(HyphaBasePlugin *plugin) {
  if (dynamic_cast<HyphaActor *>(plugin)) return true;
  return false;
}

bool PluginUtil::isSensor(HyphaBasePlugin *plugin) {
  if (dynamic_cast<HyphaSensor *>(plugin)) return true;
  return false;
}

bool PluginUtil::isReceiver(HyphaBasePlugin *plugin) {
  if (dynamic_cast<HyphaReceiver *>(plugin)) return true;
  return false;
}

bool PluginUtil::isSender(HyphaBasePlugin *plugin) {
  if (dynamic_cast<HyphaSender *>(plugin)) return true;
  return false;
}
