...
UserDBLDAP::UserDBLDAP(UserDatabaseSettings *settings)
...
bool UserDBLDAP::connect() {
  ...
  res = ldap_sasl_bind_s(ld, ldapbinddn.c_str(), LDAP_SASL_SIMPLE, &passwd,
                         NULL, NULL, NULL);
  if (res != LDAP_SUCCESS) {
    free(authpass);
    ldap_unbind_ext(ld, NULL, NULL);
...
std::list<std::string> UserDBLDAP::getUsers() {
  ...
  if (ld) {
    ...
    res = ldap_search_ext_s(ld, dbs->getDatabase().c_str(), LDAP_SCOPE_SUBTREE,
                            filter.c_str(), attrs, 0, NULL, NULL, NULL,
                            LDAP_NO_LIMIT, &msg);
...
std::string UserDBLDAP::getFirstname(std::string username) {
  if (ld) {
    UserDatabaseSettings *dbs = UserDatabaseSettings::instance();
    std::string filter("(&(objectClass=person)(" + dbs->getAttributeUsername() +
                       "=" + username + "))");
