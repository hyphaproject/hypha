...
bool UserDBSql::connect() {
  ...
  if (settings->getDriver() == "MySQL") {
    ...
    pool = new Poco::Data::SessionPool(settings->getDriver(),
...
bool UserDBSql::reconnect() { return true; }
...
std::list<std::string> UserDBSql::getUsers() {
  ...
  statement << "select " + settings->getAttributeUsername() + " from " +
                   settings->getTable();
  statement.execute();
...
std::string UserDBSql::getFirstname(std::string username) {
  std::string retValue = "";
  Poco::Data::Statement statement = getStatement();
  statement << "select " + settings->getAttributeFirstname() + " from " +
                   settings->getTable() + " where " +
                   settings->getAttributeUsername() + " = '" + username + "'",
      into(retValue);
  statement.execute();
  return retValue;
}
...
std::string UserDBSql::getLastname(std::string username) {
  std::string retValue = "";
  Poco::Data::Statement statement = getStatement();
  statement << "select " + settings->getAttributeLastname() + " from " +
                   settings->getTable() + " where " +
                   settings->getAttributeUsername() + " = '" + username + "'",
      into(retValue);
  statement.execute();
...
std::string UserDBSql::getMail(std::string username) {
  std::string retValue = "";
  Poco::Data::Statement statement = getStatement();
  statement << "select " + settings->getAttributeMail() + " from " +
                   settings->getTable() + " where " +
                   settings->getAttributeUsername() + " = '" + username + "'",
      into(retValue);
  statement.execute();
...
std::list<std::string> UserDBSql::getDevices(std::string username) {
  Poco::Data::Statement statement = getStatement();
  statement << "select " + settings->getAttributeDevices() + " from " +
                   settings->getTable() + " where " +
                   settings->getAttributeUsername() + " = '" + username + "'";
  statement.execute();
  Poco::Data::RecordSet rs(statement);
...
std::string UserDBSql::getOwnerOfDevice(std::string device) {
  ...
  statement << "select " + settings->getAttributeUsername() + " from " +
                   settings->getTable() + " where " +
                   settings->getAttributeDevices() + " like '%" + device + "%'",
      into(retValue);
  statement.execute();
  return retValue;
}

bool UserDBSql::createUser(std::string username, std::string firstname,
                           std::string lastname, std::string mail) {
  Poco::Data::Statement statement = getStatement();
  statement << "INSERT INTO " + settings->getTable() + "(" +
                   settings->getAttributeUsername() + ", " +
                   settings->getAttributeFirstname() + ", " +
                   settings->getAttributeLastname() + ", " +
...
bool UserDBSql::updateUser(std::string username, std::string firstname,
                           std::string lastname, std::string mail,
                           std::string devices) {
  Poco::Data::Statement statement = getStatement();
  statement << "UPDATE " + settings->getTable() + " SET " +
                   settings->getAttributeFirstname() + "=?, " +
                   settings->getAttributeLastname() + "=?, " +
                   settings->getAttributeMail() + "=?, " +
                   settings->getAttributeDevices() + "=? WHERE " +
                   settings->getAttributeUsername() + " = ?;",
...
void UserDBSql::createTables() {
  if (settings->getDriver() == "MySQL") {
    ...
    statement << "CREATE TABLE IF NOT EXISTS `" + settings->getTable() + "` (" +
                     ...
                     settings->getAttributeMail() +
                     "` varchar(128) DEFAULT NULL, " + "`" +
                     settings->getAttributeDevices() +
                     "` varchar(1024) DEFAULT NULL, " + "PRIMARY KEY (`" +
                     settings->getAttributeUsername() + "`)" +
